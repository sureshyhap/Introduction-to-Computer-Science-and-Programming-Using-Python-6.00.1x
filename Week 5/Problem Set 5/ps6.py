...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in lower:
            mapping[letter] = lower[(lower.index(letter) + shift) % 26]
        for letter in upper:
            mapping[letter] = upper[(upper.index(letter) + shift) % 26]
        return mapping
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            correct_words = 0
            plain = Message.apply_shift(self, i)
            plain_list = plain.split()
            for word in plain_list:
                if (is_word(self.valid_words, word)):
                    correct_words += 1
            num_correct_list.append(correct_words)
        best_shift = num_correct_list.index(max(num_correct_list))
